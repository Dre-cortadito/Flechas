...
const html = fs.readFileSync(path.join(__dirname, 'flechas', 'clasico.html'), 'utf8');
...
for (const seed of seeds) {
  ...
  // 2026-07-12: colour flows through the --ac custom property (one var feeds .pline stroke
  // and .phead fill), so we check that mechanism instead of per-element stroke rules.
  ...
  if (!/\.piece\.firing \{ --ac:var\(--clear\)/.test(html)) probs.push('firing --ac not green');
  if (!/\.piece\.blocked \{ --ac:var\(--error\)/.test(html)) probs.push('blocked --ac not red');
...
// ---------- viewport math (v8 zoom/pan) — REMOVED 2026-08-20 ----------
// This block tested VIEW_FULL / VIEW_HOME / VIEW_START / clampView / zoomAt /
// panBy / isTap / isHomeView / hintViewAt / initGestures. NONE of those exist
// in Flechas any more — the zoom/pan gesture subsystem was taken out (v11
// reworked Clasico to the 14x16 Cascada format). Grep confirms 0 hits for all
// eleven identifiers across clasico.html, cascada.html and index.html.
//
// It was NOT removed because it was inconvenient: it was testing code that is
// gone, so it could only ever report "module not found". Deleting it here is
// what lets the render-path checks above actually run and block a bad deploy.
//
// To recover it if zoom/pan ever returns:
//     git show 832d828^:_smoke-clasico.js
// (lines 150-199 of that file). Do not re-enable it as a silent skip - a check
// that quietly passes when its target is missing is how this guard rotted in
// the first place.
